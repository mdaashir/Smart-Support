...
from src.config import CATEGORIES, MODEL_DIR, URGENCY_WEBHOOK_THRESHOLD
...
async def lifespan(app: FastAPI):
    ...
    # M2: async broker
    from src.routing.broker import AsyncBroker
    broker = AsyncBroker()
    ...
    from src.routing.deduplicator import SemanticDeduplicator
    ...
    if variant == "distilbert":
        try:
            from src.models.tfidf_logreg import TfidfLogRegClassifier
            from src.routing.circuit_breaker import CircuitBreaker
...
def _route_ticket_full(subject: str, body: str) -> dict:
    ...
    from src.preprocessing.text_cleaner import combine_fields
    import uuid
    ...
    # Urgency
    from src.routing.urgency import urgency_label
    ...
    if urgency_score > URGENCY_WEBHOOK_THRESHOLD:
        ...
        try:
            import asyncio
            from src.routing.webhook import fire_webhook
